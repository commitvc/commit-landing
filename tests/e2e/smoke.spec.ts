...
test('CLI ls and the file-tree paint stealth companies alike', async ({ page }) => {
  ...
  // for live. The CLI class (`.green-faded` in globals.css) and the tree's
  // (`.fileStealth`) are separate declarations that could drift.
  await page.goto('/companies/');
  const green = await resolveVar(page, '--green');
  const greenFaded = await resolveVar(page, '--green-faded');
  expect(greenFaded, '--green-faded must differ from --green').not.toBe(green);

  const tree = await txtEntryColours(page, 'button');
  expect(Object.keys(tree).length, 'tree files').toBeGreaterThan(0);
  ...
  expect(Object.keys(cli).sort(), 'listings must match').toEqual(Object.keys(tree).sort());

  for (const [name, colour] of Object.entries(cli)) {
    expect(colour, `${name} must match the tree`).toBe(tree[name]);
    expect([green, greenFaded], `${name} must be a company green`).toContain(colour);
  }

  // Both states have to be represented, else the parity assertions pass
  // vacuously the day everything is one colour.
  expect(Object.values(cli), 'some stealth entry').toContain(greenFaded);
  expect(Object.values(cli), 'some live entry').toContain(green);
});
